...
### 2026-08-09 — Q5-B-0 승인에 따른 테스트 가드 축소 (범위 변경 기록)

인수 시점의 테스트는 "`q5b`가 이름에 든 파일이 하나도 없을 것"을 강제했다.
사용자가 Q5-B 진행을 승인한 뒤, 그 가드는 **개입(intervention)** 이 아니라
**측정**까지 막는 과잉이 됐다 — Q5-B-0(EXP-2026-005)은 D5의 사전등록 next_step이
지시한 "가장 저비용의 추가 측정"이고 학습을 하지 않는다. 그래서 가드를
`q5b1`/`cvar`/`groupdro` 로 좁히고, 대신 **Q5-B-0 모듈이 실제로 학습 호출을 담고
있지 않은지**를 같은 테스트에서 검사하도록 바꿨다. 금지의 대상은 파일 이름이
아니라 학습이므로 이 축소는 규칙을 약화시키지 않는다. Q5-B-1(개입 pilot)은 여전히
승인 전까지 만들지 않는다.

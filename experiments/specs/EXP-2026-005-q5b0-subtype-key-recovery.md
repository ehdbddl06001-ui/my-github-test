...
run_id: 20260809T1241_EXP-2026-005_q5b0b_subtype_key_recovery_tie
measured: 2026-08-09
...
**상태: MEASURED (2026-08-09) — `B_SUBTYPE` 영구 종결.** 두 번의 사전 등록이
모두 `NO_GO_SUBTYPE_CLOSED`로 끝났다: Q5-B-0(`runs/20260809T1219_…_q5b0_subtype_
key_recovery`, 조인 25.9%)과 Q5-B-0b(`runs/20260809T1241_…_q5b0b_subtype_key_
recovery_tie`, 조인 90.5%이나 드문 subtype 회수율 0.33으로 편향). 0~12절은 **측정 전에 등록된 설계 원문**이며 결과를 보고 고치지 않았다.
...
## Q5-B-0b 결과 (2026-08-09 실측 · MEASURED) — `NO_GO_SUBTYPE_CLOSED` 확정

run `20260809T1241_EXP-2026-005_q5b0b_subtype_key_recovery_tie` · 학습 없음.

**규칙은 의도대로 작동했고, 바로 그 덕분에 이 산출물로는 안 된다는 것이
확정됐다.** `B_SUBTYPE`은 **영구 종결**이다. 사전 등록대로 세 번째 규칙은 만들지
않는다.
...
| | Q5-B-0 | **Q5-B-0b** |
|---|---|---|
| 조인된 S beat | 721 (25.9%) | **2,516 (90.5%)** |
| 동점 집합 symbol 일치율 | — | **99.3%** (1,923박 해소) |
| symbol이 AAMI S 집합 | 100% | **100%** |
| wrong-record 영가설 | 0.48% | **0.71%** |
| 신호/영가설 | 54배 | **127배** |

영가설은 예상대로 올랐지만(0.48% → 0.71%) 기준(≤5%) 안이고, 신호비는 오히려
올랐다. **규칙 완화의 대가는 오매칭이 아니었다.**

### 그런데 결정적으로 실패한 것 — subtype별 회수율

| subtype | 회수/참값 | 회수율 | 전체 대비 |
|---|---|---|---|
| A | 2395 / 2546 | 0.941 | 1.04 |
| **a** | **50 / 150** | **0.333** | **0.368** ← 기준 0.80 미달 |
| J | 70 / 83 | 0.843 | 0.93 |
| S | 1 / 2 | (0.500) | 20박 미만 — 서술용 |

**빠진 10%는 무작위가 아니라 드문 subtype에 몰려 있다.** 이 표를 그대로 쓰면
`a`의 비중이 참값 5.4%에서 **2.0%로, 즉 0.37배로 축소**된다. 그러면 `B_SUBTYPE`은
"S 하위분류"가 아니라 사실상 **"A인가 아닌가"** 를 재게 되고, 하필 드문
subtype이 더 어려운 쪽일 가능성이 있으므로 편향의 방향이 최악이다.

기전은 사전 등록 때 예측한 그대로다: `a`를 포함한 동점 집합은 대개 다른
구성원이 A라 **혼합**이고, 그러면 규칙이 (올바르게) 부여를 거부한다. 이것은 더
똑똑한 규칙으로 풀 문제가 아니다 — **동일성을 주장해야만** 풀리는데, 그게 바로
이 산출물이 지탱하지 못하는 것이다.

또 하나 남는 사실: 90.5%까지 올라와도 **실제로 식별된 beat는 여전히 19.8%
(551박)** 뿐이다. 나머지는 "누구인지는 모르지만 symbol은 정해진다"이며, 그
구분은 끝까지 기록에 남는다.

### 판정

`s_match_fraction` 0.9047(<0.95) · record floor 21/32 · `content_anchor_fraction`
0.1981 · **`subtype_coverage_balance` 0.3684(<0.80)** 로 `NO_GO_SUBTYPE_CLOSED`.
symbols를 붙이지 않았고 재분석도 돌리지 않았다.

**`B_SUBTYPE`은 여기서 닫는다.** Q5-A의 `UNRESOLVED`(D5)는 4개 블록 위에서
확정 유지되며, 그 4개 블록에는 이제 "다섯 번째는 왜 없는가"에 대한 실측 답이
붙어 있다. 남은 다음 후보는 Q5-B-1(개입 pilot)이며 별도 승인 대상이다.

## Decision log

### 2026-08-09 — Q5-B-0b 실측: 규칙은 작동했고 데이터가 답을 줬다 (결과 기록)

조인률은 25.9% → 90.5%로 올랐고 영가설·신호비·symbol 검증은 모두 개선되거나
유지됐다. 그럼에도 `subtype_coverage_balance`(0.368)가 탈락시켰다 — 남은 10%가
드문 subtype(`a` 회수율 0.333)에 몰려 있어 블록이 편향되기 때문이다.

**여기서 규칙을 한 번 더 고치지 않는다.** 사전 등록에 "실패하면 그때 영구
종결, 세 번째 규칙은 만들지 않는다"고 적었고, 숫자가 좋아졌다는 것은 그 약속을
깰 이유가 되지 못한다. 오히려 이번 gate는 **"많이 붙었다"와 "쓸 수 있다"가 다른
것임을 실측으로 구분해 준 사례**로 기록한다.

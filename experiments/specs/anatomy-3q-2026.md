...
- **D8**: 브랜치는 하네스 지정 `claude/anatomy-3q-2026-okgos4`를 사용(프롬프트의
  ...
  규약은 동일).
- **D9 (2026-08-12, 사용자 요청)**: 그림 자료 정책. ① 외부 이미지 다운로드는 프록시
  차단 실측(Wikimedia 포함) + 상용 해부도 저작권으로 불가. ② 사용자가 PDF를 세션에
  직접 업로드하면 binary lane 전체가 작동(마스킹 포함) — 단 파생 이미지의 공개는
  사람이 결정. ③ 기본 공개 경로는 **클로드 자체 제작 SVG 모식도**(`asset_origin:
  claude-drawn-svg`): 출처에서 확인된 관계·행선지만 담고, 라벨판/번호핀 퀴즈판 분리.
  해부1 실측: 김홍태·허미선 회차는 필기 스캔(텍스트 희소, needs_review), 문용석
  회차는 타이핑 텍스트 풍부, 6회차(57MB)는 MCP 추출 빈 응답(binary lane 전용).
- **D10 (2026-08-12)**: 사용자 업로드 「2026-2학기 수업시간표.xls」 원본과
  `SCHEDULE_2026` 16개 일자·주제 전량 대조 — **완전 일치 확인**(수정 불필요).

...
## 자주 나는 문제
- **옛 노트북을 열었는데 다른 앱(TrialMatch)이 뜬다**: 같은 Colab 런타임에서 두 앱을 돌리면
  파이썬이 먼저 로드한 모듈(`server`)을 캐시해 옛 파일을 다시 안 읽습니다. 이 노트북은 전용
  폴더(`oncoreg_ai/`)·전용 모듈명(`oncoreg_app`)·전용 포트(8000)로 분리했고 맨 앞에 캐시
  정리 셀(0단계)을 뒀습니다. 그래도 섞이면 **런타임 → 세션 다시 시작** 후 이 노트북만 위에서
  부터 실행하세요(포트 점유까지 한 번에 해소). 실행 셀이 `>>> 실행 중인 앱: OncoReg AI` 를
  출력하는지 확인하면 됩니다.

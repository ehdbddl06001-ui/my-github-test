...
1) papers: 이 케이스에 부합하는 근거 문헌 4~6개. 유사도가 서로 다른 논문을 섞어라
   ...
   - sim: 이 논문의 '대상 집단'이 위 [검토 케이스] 환자와 얼마나 비슷한지 축별 점수
     ...
     아니면 중간값(50~70) 정도로.
2) metrics: 핵심 지표. **위 papers 의 논문마다 최소 1개씩** 수치를 뽑아 그 논문 id 를 src 로
   지정한다(논문 수만큼, 최대 8개). 안전성 지표를 최소 1개 포함하고 그 k 는 "AE" 로 시작.
   - k 는 짧은 영문 키(ORR/PFS/HR/AE/REC/SUB 등), 서로 중복되지 않게.
   - src 는 반드시 해당 논문의 id (한 논문도 빠짐없이 최소 1개 지표를 갖도록 분배).
   - 진료지침·정성 근거 논문처럼 수치가 없으면 권고등급/핵심 결론을 val 로 쓰고
     (예: val="Category 1", "병용요법 권고"), mark 에는 그 근거가 되는 문장을 넣는다.

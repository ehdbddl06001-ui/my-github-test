...
| 필드 | 예시 | 설명 |
|------|------|------|
| `kind` | `project` | 카드 성격: `project`(공개 프로젝트 분석)·`roadmap`(커리큘럼)·`weekly`(주간 실습)·`concept`(개념)·`mentor`(클로드와의 논의 노트)·`deepdive`(완료 주차 심화)·`log`(실제 실행 결과 로그)·`quest`(주차 밖 독립 심화 퀘스트) |
...
**심화 퀘스트(`kind: quest`)** — 12주 커리큘럼(주차)과 **직교하는 독립 심화 트랙**. 어떤 실습에서
나온 발견(예: inter-patient에서 S·F 붕괴)이 **한 주차 숙제로 끝낼 수 없는 열린 문제**일 때, 그걸
별도 퀘스트로 큐에 넣는다(논문급 방법 — SMOTE·도메인적응·self-supervised·파운데이션 모델 등을
제대로 다룬다). 주차 게이트(metric·target)가 없고 **로드맵과 실험 큐**를 담는다.
`content/ailab/quests/`에 쌓이며, `week`에 얽매이지 않는다(있어도 '출발점 주차' 참조용).
`/gen-quest` 스킬이 만들고, 홈페이지 🤖 AI랩 '실습 진도'에 **🎯 심화 퀘스트** 블록으로 뜬다.
...
**깊이(2026-09-18 학습서 판형 2)** — 한 단원을 읽으면 「정상 기능 → 이상이 생기는 기전 → 증상·검사 소견 → 감별·기준 →
치료 선택 → 반응 확인·재평가」가 이어져야 한다. 원인·치료명 나열로 설명을 대신하지 않는다. 정상·음성 결과의 해석 한계,
수치의 단위·적용 조건, 치료의 적응·금기·예외·재평가 기준을 줄이지 않는다. 역할을 나눈다: `summary` = 전체 관계,
본문 = 이유, `tables` = 차이 비교, `diagram` = 흐름(아래에 노드를 되풀이하지 않고 `diagram_notes` 로 그림에 없는 조건만).

| 필드 | 내용 |
|---|---|
| `sources[].verified` | `text`(권고·서술 본문 대조) · `abstract`(초록만) · `citation`(서지만). text 가 아니면 근거 표시에 † |
| `sources[].locator` 없이 본문에서 | `[[출처id: 쪽·절·표]]` 로 근거 위치를 단다. `[[?출처id: …]]` = 이 주장만 원문 미대조(†). 교과서는 url 대신 판·장·쪽 `citation` |
| `criteria[].locator` | 기준의 원문 위치(예: `Guideline 16.2(p.13)`). PDF 는 기준을 `kind` 별 비교표로 싣는다 — 다른 지침의 기준은 행을 나눠 합치지 않는다 |
| `tables` | `{id, title, role(differential·treatment·criteria·severity·tests·monitoring·comparison), span(full·column), section(이 표를 둘 본문 절 제목), columns, rows, note}`. 한 단 표는 4열, 전체 너비는 6열까지 — 넘으면 역할별로 나눈다. 같은 대상·출처는 `note` 에서 한 번 |
| `pitfalls` | 혼동하기 쉬운 점 `{contrast, point, exception, cites, covers:[문항id:보기]}` — 오답에서 드러난 혼동을 **일반화**한다. 학습자가 왜 골랐는지 추측하거나 「모른다」고 쓰지 않는다. `covers` 에 없는 오답은 학습서가 문항의 `discriminator`·`when_right` 로 자동 보탠다 |
| `diagram_notes` | 도식만으로 전달되지 않는 조건·예외(문장 목록) |

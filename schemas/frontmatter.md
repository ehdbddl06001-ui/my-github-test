...
| 필드 | 예시 | 설명 |
|------|------|------|
| `kind` | `project` | 카드 성격: `project`(공개 프로젝트 분석)·`roadmap`(커리큘럼)·`weekly`(주간 실습)·`concept`(개념) |
...
| `week` | `28` | 주간 실습이면 ISO 주차(중복·순환 관리) |

본문 섹션 관례(웹 피드가 파싱): `## Overview`(한눈에)·`## Architecture`(구조 도식,
Mermaid/ASCII)·`## Data`(데이터·전처리)·`## Code walkthrough`(핵심 코드)·
`## Instructions`(지시어/프롬프트가 뭘 시키는지 해설)·`## Exercises`(직접 해볼 것)·
`## Resources`(공부 자료). `confidence`는 공식 문서 기반이면 `high`, 정리·해석이 섞이면
`medium`으로 둔다. 결정론적인 것(주차 선정·데이터셋 목록)은 `pipelines/datasets.py`가 맡고,
카드는 해석·학습 노트를 담는다.

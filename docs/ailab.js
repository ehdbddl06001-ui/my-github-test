...
window.AILAB = {
 ...
 "cards": [
  ...
  },
  {
   ...
   "nextGoal": "실험1′·2·3·4·9·9b·10·10′·10″·13·13b 종결. **실험13b: 차별점 A의 표가 나왔다** — 소견 14개를 잔여 결손 `D = AUC({12}) − AUC({I,II})` 로 정렬. 전두면 평균 −0.008 vs 횡단면 +0.053, P-1 Δ+0.0601 [+0.0459,+0.0739] 확증. **P-3에서 CD가 독립 복제**(속가지차단 vs 각차단)돼 평면 구분이 MI·STTC·CD 세 패널에서 재현됐다. 예상 못 한 발견: **완전 vs 불완전 우각차단이 D 0.003 대 0.130으로 40배 갈렸다** — 완전형은 `QRS ≥ 120 ms`라는 평면무관 단서로 잡히고, 불완전형은 V1 형태 말고 근거가 없어 `{II,V1}`이 결손의 96%를 메운다. → **평면 배정은 소견 이름이 아니라 '모델이 쓸 수 있는 단서'로 해야 한다**(CRBBB·CLBBB는 다음 판에서 혼합으로 재배정; 이 오배정 때문에 현재 P-1은 보수적 통과다). P-2는 노트북이 '기각'으로 찍었으나 **표기 버그**여서 3분으로 고쳤다(p=0.0714는 상위 7%라 약한 지지). LVH의 D=+0.059는 세 번째 측정에서도 앵커의 53%로 중간. **단계 로드맵을 확정했다**: 1계측(거의 완료, 실험14만) → 2라벨 공사(SCP→finding 매핑·다중라벨 전환, 미착수·큐 0번) → 3 finding head(실험15가 최소 실행판) → 4임상 동작점·배포. 지금까지 실험은 전부 계측기를 만들고 검증한 것이고 5-superclass는 제품이 아니다. 다음은 **실험14(임상 동작점 곡선 — 학습 0회로 MI 민감도 0.90/0.95의 특이도·NPV 실측)** → **실험15(MI 부위 다중라벨 헤드 — 첫 finding head, GPU 1시간)** → 실험9c(샘플링 비율 스윕 + seed 3개 재현). 후벽경색은 n=7이라 어떤 리샘플링으로도 안 되고 '탐지 불가'로 명시 보고한다. 실험9b 결론: 저유도 붕괴는 균등 샘플링 부작용({II} 40% 샘플링으로 −0.0735 → −0.0235). 실험3: 형태축−리듬축 분리 확증. 실험4: 기권 신호는 클래스마다 달라야 한다. 판단신호 미검정 5개의 시험장은 ailab-2026-0018 참조",
   "status": "active",
   "sections": {

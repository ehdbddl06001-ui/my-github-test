...
window.AILAB = {
 ...
 "cards": [
  ...
  },
  {
   ...
   "nextGoal": "실험1′·2·3·4·9·9b·10·10′·10″·13·13b·14 종결. **실험14: 재현율 우선의 대가는 특이도가 아니라 경보율이었다** — MI 민감도 0.90 고정 시 {12} 특이도 0.605 [0.569,0.640](P-1 미결, CI가 하한 0.6을 걸침)인데 **경보율이 47.3%**, 웨어러블 {II}는 64.8%다. 들어온 심전도의 절반을 올린다는 뜻이라 현장에서 죽는 건 이 숫자다. P-2는 {I,II} 0.501 vs {12} 0.605로 −0.104, 선을 **0.004** 넘겨 기각됐는데 **P-2만 CI 없이 점추정 2분으로 채점한 내 설계 결함**이다(13b P-2와 같은 실수) → 앞으로 모든 관문은 CI 기반 3분으로 통일. P-3 지지(낙관 편차 최대 +0.0355, 저유도일수록 큼). **경고가 좋은 소식으로 뒤집혔다**: 급성 손상이 사는 STTC 쪽이 오히려 낫다({12} 경보율 31.8%, 웨어러블 격차 −0.070으로 P-2 기준 통과) → **ACS 트랙의 1차 타깃은 MI superclass가 아니라 STTC여야 한다.** **R1을 고쳤다**: F-β 최대화는 민감도 목표를 겨냥하지 못하므로(β=2 sens 0.773 / β=5 spec 0.361) **제약 하 최적화**를 표준으로 하고, **경보율**과 **NPV의 항상-음성 기준선(=1−유병률)** 을 반드시 함께 보고한다(p=5%에서 NPV 0.991은 기준선 0.950 대비 이득 +0.041일 뿐). 다음은 **실험15(MI 부위 다중라벨 헤드 — 첫 finding head)**: 전량 캐시를 새로 만들고(기존 캐시 보존) 유도 2구성 × 5겹 = 10회 학습. 백본은 실험10′ 그대로 두고 헤드와 손실만 바꾼다(한 번에 하나). CELL 3 인구조사에서 GPU를 켤지 정할 수 있게 짰다. P-3(잔여 결손의 평면 순서가 학습 타깃이 바뀌어도 유지되는가)이 숨은 본체 — 유지되면 차별점 A에서 '이 모델 기준' 단서를 뗄 수 있다. 그다음 15b(pos_weight) · 15c(다중라벨 공유의 값어치) · 9c(샘플링 비율 스윕 + seed 재현) · 4c. 후벽경색은 어떤 리샘플링으로도 안 되고 '탐지 대상 아님'으로 명시 보고한다. 판단신호 미검정 5개의 시험장은 ailab-2026-0018 참조",
   "status": "active",
   "sections": {

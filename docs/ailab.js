...
window.AILAB = {
 ...
 "cards": [
  ...
  },
  {
   ...
   "nextGoal": "실험1′·2·3·4·9·9b·10·10′·10″ 종결. **실험10″: 쪼갤 것이 없었다** — PTB-XL의 HYP 535건 중 LVH 순수 456건(85.2%), RVH는 2건. `HYP`는 사실상 `LVH`의 다른 이름이고, '65%는 RVH가 희석한 값'이라는 갈래는 계수만으로 사망했다(2건이 535건을 희석할 수 없다). LVH 비중 57.1% [41%,74%]로 P-A는 미결. **배운 것 하나: 비중은 지표 불변량이 아니다** — 같은 LVH 레코드에서 46.6%(재현율)/57.1%(AUC)/65%(group-F1)로 갈린다. 앞으로 기준선은 같은 실행·같은 지표로 계산한다(P2 기각의 **방향**은 세 지표 모두에서 살아남았다 — 전부 33%를 크게 넘음). 사전등록 밖 관찰: 심방확대의 HYP AUC가 전 구성 0.5 미만(P파 소견과 QRS 전압 소견을 한 라벨에 넣은 결과 → finding-first 원칙의 직접 증거)이고 `{II,V1}`에서만 재현율이 튄다(0.191, n=47이라 가설로만). 다음은 **실험13(소견 평면 대조 — 하벽경색 IMI vs 전중격경색 ASMI, 학습 0회 재분석)**: HYP에서 못 찾은 순수 횡단면 대조군을 MI subclass에서 찾는다. IMI는 II·III·aVF(전두면), ASMI는 V1–V4(횡단면)로 해부학적 예측이 교과서이고 표본이 수백 건대라 검정력이 있다. 그다음 실험9c(샘플링 비율 스윕 + seed 3개 재현) · 실험4c(RR 넣은 분류기의 SNR별 S 곡선). 실험9b 결론: lead-agnostic의 저유도 붕괴는 균등 샘플링의 부작용이었다 — `{II}` 40% 샘플링으로 −0.0735 → −0.0235, 4구성 절대손실 합 0.1034 → 0.0554. 한 가중치로 웨어러블·텔레메트리를 함께 서비스하는 전략이 성립한다(단 H2의 `{12}` 열화 CI가 합격선을 걸치고 seed는 1개). 실험10′ 결론: 혼합군(MI) 교호작용 +0.0859 [+0.0672,+0.1052] 확증. 실험3 결론: 형태축−리듬축 분리 M−R +0.2862 확증. 실험4 결론: 기권 신호는 클래스마다 달라야 한다(확률의 S 내부 AUC 0.184 역전). 판단신호 미검정 5개의 시험장은 ailab-2026-0018 참조",
   "status": "active",
   "sections": {

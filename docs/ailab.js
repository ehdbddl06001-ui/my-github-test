...
window.AILAB = {
 ...
 "cards": [
  ...
  },
  {
   ...
   "nextGoal": "실험1′·2·3·4·9·10·10′ 종결. 실험10′ 결론: 라벨군별 유도 이득 차이는 **혼합군(MI)에서 확증**(+0.0859 [+0.0672,+0.1052]). 횡단면군(HYP)은 비유의인데 가설이 틀려서가 아니라 HYP F1 천장이 0.21로 낮아서다. 그리고 P2 실패로 **HYP가 순수 횡단면군이 아님**이 드러났다(Cornell voltage가 aVL 사용 → ailab-2026-0016 2-bis 수정 완료). 다음은 실험9b(lead-agnostic이 {II}에서 붕괴 — 세 번 독립 확인) · 실험4c(RR 넣은 분류기의 SNR별 S 곡선) · 실험10″(HYP subclass 분해). 실험10 결론: 평균은 {I,II}가 12유도의 88.8%라고 말하지만 HYP 환자에게는 71.9%다 — 라벨군을 나누면 격차가 드러난다. 다만 교호작용은 HYP 테스트 54건 탓에 CI가 0을 걸쳐 미결. 다음은 실험10′(전체 실행으로 검정력 확보) · 실험9b(lead-agnostic이 {II}에서 −0.178로 붕괴하는 문제) · 실험4c(RR 넣은 분류기의 SNR별 S 곡선). 실험4 결론: 기권 신호는 클래스마다 달라야 한다(확률은 N에서 AUC 0.878인데 S에서 0.184로 역전, 품질은 그 반대) — 예측 클래스 조건부 기권 규칙 검정을 큐에 추가. 다음은 실험10(유도 ablation, 노트북 준비완료 — 실험9 lead-agnostic 동시 수행) — 실험3이 남긴 PTB-XL lead II 파이프라인에 유도 슬라이스만 갈아끼우면 되고, MI·HYP로 뺀 7,300건(33%)을 되찾는 실험이다. 판단신호 미검정 5개의 시험장은 ailab-2026-0018 참조",
   "status": "active",
   "sections": {

...
window.AILAB = {
 ...
 "cards": [
  ...
  },
  {
   ...
   "nextGoal": "실험1′·2·3·4·9·9b·10·10′·10″·13·13b·14·15·15c·15d 종결. **실험15d: 계측기 검정 — 지금까지의 CI 가 좁았다(🔴 소급 정정 발생).** 부트스트랩은 테스트셋 잡음만 재는데 실험 9b·13·13b·14·15·15c 는 전부 **따로 학습한 두 모델**을 비교했고 `n_seeds=1` 이라 재학습 잡음이 CI 에 없었다. 시드 3개로 재니 IMI 시드SD 0.0328 / 테스트SD 0.0078 = **4.22** → 단일시드 CI 가 **4.34배 좁았다**(LMI 는 0.99 / 1.41배). **실험15c 의 유일한 유의 결과가 뒤집혔다**: IMI `{12}` Δ 가 시드별 −0.0303 / −0.0114 / **+0.0336** 으로 부호가 갈려 t-CI −0.0027 [−0.0842,+0.0788] **미결** → 15c 의 IMI 개별 우위·ASMI 공유 우위 판정을 **소급 철회**한다. 실험15 헤드라인도 하나 깨졌다: **IPLMI lift 60.9배는 운 좋은 시드**였고 중앙값 23.6배(23~61배)라 ALMI 34.6배와 구별되지 않는다. **생존 판정**: 효과 0.10 이상은 견고(실험15 P-3 횡단면−전두면 **+0.1481 = 시드잡음의 4.5배** ✅), 0.05 이하는 미결로 강등 → **이 퀘스트의 핵심 지표인 저유도 결손 D 는 무사하고, 깨진 것은 0.02~0.03 짜리 헤드 구조 비교뿐이다.** ⚠️ **시드는 짝지어도 상쇄되지 않는다** — 공유는 시드 따라 올라가고 개별은 내려가 Δ 의 SD(0.0328)가 공유 단독 SD(0.0191)보다 크다. ⚠️ **G0 실패** — 같은 시드로도 재현이 안 된다(GPU 비결정성)가 시드 분산에 섞여 있고, 실험15·15c 수치는 정확 재현 불가. **새 관찰**: 시드 SD 는 표본 수가 아니라 **소견의 모호성**을 따라간다 — ASMI(n=2357) 0.0035 vs IMI(n=2676) 0.0191 로 5.5배 차. V1–V3 Q파처럼 또렷한 표지가 있으면 어떤 모델이든 같은 곳을 보고, II/III/aVF Q파처럼 정상 변이와 겹치면 학습할 때마다 선이 달라진다(가설·미검증). **결정: 72시드는 안 돌린다** — 1,065회 학습 17시간으로 얻을 값이 −0.0027 이라 아무 결정도 안 바뀐다. **새 규약(실험16부터)**: ①결론용 비교는 최소 3시드 + 시드 수준 t-CI ②효과가 시드 SD 보다 작으면 표본 늘리지 말고 '검출 불가'로 종결 ③실험 1~15c 의 CI 에 '재학습 잡음 미포함' 단서 ④단, **두 모델을 따로 학습해 비교할 때만** 해당(같은 모델의 두 임계값·부분집합 비교는 무관). 역설적으로 15c 의 답은 깨끗해졌다 — '미결이니 단순한 쪽'이 아니라 **'차이가 재학습 잡음보다 작다 → 실무적으로 없다 → 공유 채택'**. 논문에서는 결함이 아니라 **결과**다(ECG 딥러닝 논문 대부분이 single-seed). 다음은 **실험16(유도 4구성 확장 — `{II}`·`{I,II}`·`{II,V1}`·`{12}` × 3시드 × 5겹 = 60회, ~60분)**. 핵심은 `{I,II}` 와 `{II,V1}` 이 **전극 수가 같고 두 번째 전극의 평면만 다르다**는 것이다 — 소견에 따라 우열이 뒤바뀌면(P-4) 차별점 A 가 '설명'에서 **전극 배치 규칙**으로 승격된다. V1 하나가 전벽 결손의 얼마를 되찾는지(P-3)가 웨어러블 설계의 실제 답이다. 실험15 arm 은 **재사용하지 않는다** — 15d G0 에서 환경 드리프트가 확인됐고 헤드라인이 두 구성의 교차라 한쪽 팔만 다른 세션이면 드리프트가 그 위에 얹힌다(대신 드리프트 크기를 G0-b 로 남긴다). 그다음 **15b(pos_weight — LMI 특이도 0.510·AMI 0.554, 3시드 규약 + 라벨 잡음 점검)** → 9c · 4c. 논문 전 필수는 **외부검증(Chapman/Ningbo)·시드 반복(15d 로 착수)·급성 코호트(MIMIC-IV-ECG)** 셋. 판단신호 미검정 5개의 시험장은 ailab-2026-0018 참조",
   "status": "active",
   "sections": {

...
window.AILAB = {
 ...
 "cards": [
  ...
  },
  {
   ...
   "nextGoal": "실험1′·2·3·4·9·9b·10·10′·10″·13·13b·14·15·15c·15d·16·17·18 종결. **실험18: 승자는 5전극 `{I,II,V2,V5}` — 최악 D +0.0076·중앙값 +0.0030·경보배수 1.03.** 7부위 전부 12유도의 0.008 AUROC 이내이고 ALMI(−0.012)·LMI(−0.016)는 오히려 낫다. V2 가 V1 을 이기는 자리는 둘: ASMI +0.0267→+0.0060(전중격은 V2 에서 R파 감소가 가장 뚜렷), IPLMI +0.0286→+0.0030(후벽은 V1–V3 의 거울상). 반면 사전등록 MAIN 이던 4전극 `{I,II,V1}` 은 경보배수 중앙값 1.30 으로 관문 1.25 초과 — AUROC 는 괜찮은데 동작점에서 걸린다. **A0 ✅ 정정**: IMI 초과 +0.0108 [−0.0209,+0.0425] 미결 → 실험17 P-1 의 점추정 기각을 철회한다. **⚠️ A1 철회(내 구현 결함)**: 라우팅 쪽만 시드 3개 예측을 평균하고 기준은 시드별 AUROC 의 평균이라 앙상블과 교란됐다. 결정적 증거로 7부위 중 5부위가 전 겹 동일 구성을 골랐는데도(라우팅이 한 일이 없다) D 가 개선됐다 — LMI 는 −0.0218→−0.0688 로 0.047 이 전부 앙상블이다. **대신 발견: 시드 3개 예측 평균만으로 AUROC +0.0256**(중앙값, 범위 +0.013~+0.067) — 이 퀘스트가 쫓아온 어떤 유도 효과보다 크고 배포에서 순전파 3번이면 되는 사실상 공짜다. **민감도 판단이 이 실험의 진짜 결론이다**: 민감도 0.90 을 PPV 로 번역하면 쓸 수 있는 부위는 ASMI 45.7%·IMI 35.3% 둘뿐이고 ILMI 10.8%·ALMI 6.4%·AMI 3.2%·LMI 1.8%·IPLMI 0.4%(경보 1건당 위양성 251건)다. **그리고 12유도도 똑같다**(ASMI 48.4·IMI 34.5·LMI 1.8·IPLMI 0.6) → **유도 최적화는 천장에 닿았다.** 남은 병목은 유도가 아니라 **유병률 × 동작점 설계**다. 다음은 **실험19(동작점 2단계 재설계 — 실험16~18 arm 재사용 + 30회 ~50분)**. 임상 순서는 ①MI 유무(놓치면 죽는다 · 민감도 0.95) ②부위(이미 ①에서 잡혔으니 특이도 쪽으로)인데 지금은 부위마다 민감도 0.90 을 걸어 놓쳐도 되는 것에 값을 치른다(IPLMI 경보율 52%). ⚠️ 픽스처가 미리 잡은 함정: 두 단계는 독립이 아니라 0.95×0.90=0.855 로 계산하면 틀린다. `any` 게이트(부위 확률의 최댓값)는 **흔한 부위가 임계값을 정해** 희소 부위 양성례를 21~24% 버리고, `safe` 게이트는 민감도를 지키는 대신 아무것도 못 거른다 → **MI-any 전용 이진 헤드를 따로 학습**해(15회) 게이트 3종을 다 재고 고른다. P-1 `{I,II,V2,V5}` 6시드 D<0.02 · P-2★★ 같은 민감도에서 희소 4부위 경보 감소 · P-3★ 희소 PPV 2배 · P-4 앙상블 이득 양쪽 공정 비교 · P-5 라우팅 재측정(❌면 실험20 불필요). 그다음 15b(pos_weight + LMI 라벨 잡음) → 9c · 4c, 조건부로 실험20(평면 분리 헤드). 논문 전 필수는 **외부검증(Chapman/Ningbo)·시드 반복(15d·18 로 착수)·급성 코호트(MIMIC-IV-ECG)** 셋이고, 실험18 이후로는 **급성 코호트가 특히 중요해졌다** — 유병률이 높으면 PPV 가 따라 오르기 때문이다. 판단신호 미검정 5개의 시험장은 ailab-2026-0018 참조",
   "status": "active",
   "sections": {

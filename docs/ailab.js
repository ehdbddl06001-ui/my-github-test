...
window.AILAB = {
 ...
 "cards": [
  ...
  },
  {
   ...
   "nextGoal": "실험1′·2·3·4·9·9b·10·10′·10″·13·13b·14·15·15c 종결. **실험15c: 미결 — 그런데 어디를 봐야 하는지가 바뀌었다.** 공유 헤드 vs 부위별 개별 분류기에서 P-1·P-2·P-3 전부 미결이다. |Δ| 중앙값 0.021 vs CI 반폭 0.038 로 **효과 자체가 잡음 규모**라 표본을 늘려서 될 일이 아니다. 그래도 구조는 보인다: P-3 `{12}` +0.0440 [+0.0155,+0.0746] ✅ 로 희소 부위가 공유로 이득을 보는 기전이 12유도에서 나타나고, `{I,II}` 에서는 사라진다 → **유도가 적어 어려우면 공유가 정칙화로 작동하고(ASMI `{I,II}` +0.0228 ★), 많으면 용량 경쟁이 드러난다(IMI `{12}` −0.0301 ✗).** 방향이 구성에 따라 갈리므로 \"공유가 낫다/나쁘다\"를 하나로 말할 수 없다. **결정적 발견은 방법론 쪽이다**: 부트스트랩이 테스트셋 잡음만 재는데 비교 대상은 두 **학습 절차**이고 `n_seeds=1` 이라 **재학습 잡음이 CI에 아예 없다** — 유일하게 유의했던 IMI `{12}` −0.0301 조차 시드 산물일 수 있다. **그래서 다음은 소견 수가 아니라 시드다(실험15d).** 또 8칸 중 6칸에서 개별이 특이도 우위라 AUPRC 와 임상 동작점이 어긋난다 — 배포에 쓰는 건 동작점 한 점이므로 헤드 구조 결정에 함께 본다. 부록 성과: **실험15 P-3 이 정량화됐다 — 횡단면 − 전두면 +0.1481 [+0.1306,+0.1646] ✅, 효과가 CI 폭의 10배**라 차별점 A 의 근거가 확정된다. macro-AUPRC 0.3755 는 유병률 50배 차이를 평균한 값이라 macro-lift 20.7배·macro-AUROC 0.900 과 함께 봐야 한다(AUPRC 로는 IPLMI 가 꼴찌인데 lift 로는 1위로 순위가 뒤집힌다). 사전등록대로 공유를 **잠정** 채택하되 \"차이를 못 보였으니 단순한 쪽\"이라는 뜻이다. 다음은 **실험15d(시드 3개 — IMI·LMI × {12}, 45회 ~50분)** → 15b(pos_weight, LMI 특이도 0.510·AMI 0.554 가 여전히 못 쓸 수준) → 유도 4구성 확장 → 9c · 4c. 논문 전 필수는 **외부검증(Chapman/Ningbo)·시드 반복·급성 코호트(MIMIC-IV-ECG)** 셋이고, 군 CI·유병률 정규화는 저장된 산출물로 재계산하면 되는 것들이다. 판단신호 미검정 5개의 시험장은 ailab-2026-0018 참조",
   "status": "active",
   "sections": {

...
window.AILAB = {
 ...
 "cards": [
  ...
  },
  {
   ...
   "nextGoal": "실험19 실행 중. **외부 검토를 받아 종착점을 바꿨다 — 목표는 성능이 아니라 비교가능성이다.** 가장 중요한 지적: **우리는 질병이 아니라 판독문을 예측하고 있다.** PTB-XL 의 ASMI 같은 라벨은 심전도를 읽어서 만든 라벨이라, 이 과제는 '심전도로 심전도 판독문을 예측' 하는 구조이고 천장은 질병의 존재가 아니라 **판독자 간 일치도**다. 상용 OMI 모델(조영술+트로포닌 확진, 급성 폐색)과는 **과제가 달라 숫자 비교가 성립하지 않는다.** → 모든 카드·발표에 **'본 모델은 급성 심근경색이 아니라 PTB-XL 의 심전도 판독 라벨을 예측한다'** 를 명시한다. 또 우리 수치를 특이도로 역산하면 ASMI 88.4% · **IMI 76.0%**(정상 4장 중 1장에 경보)로 낮다 — 앞으로 경보율과 특이도를 **둘 다** 보고한다. **사실관계 정정 2건**: ①검토가 'AUC 는 전부 NORM 대조' 라 했으나 실험15 이후 코드는 `y = Ymul[:,j]` 로 **음성군이 전량 21,799건**이다 (LBBB·LVH·WPW 가 이미 음성군에 있고, 지적된 AUC 0.881 은 우리 로그에 없는 값 — 실험15 macro-AUROC 는 0.9002). ②공개 벤치마크 0.93 대역은 `superdiagnostic` 5클래스 · fold 1–8/9/10 프로토콜 값이라 **우리(7부위 국소화·5겹 CV)와 과제도 분할도 다르다** — 우열을 말하려면 같은 프로토콜로 재야 한다. **다만 지적의 알맹이는 살아 있다**: 감별질환이 음성군에 섞여 있을 뿐 **희석**돼 있어 CLBBB 에서 위양성 100% 여도 전체 AUROC 는 안 움직인다 — **감별질환별 위양성을 분해해 본 적이 없다.** **남은 순서를 재정의했다**: ①실험19(2단계 동작점, 30회) → ②**실험22 감별질환별 위양성 분해(학습 0회)** — CLBBB·ILBBB·CRBBB·IRBBB·IVCD·LVH·RVH·SEHYP·WPW·LAFB/LPFB 별로 위양성을 쪼개고 사전등록은 '병적 Q파·R파 진행 불량군의 위양성률 ≥ 2 × NORM' → ③**실험20 PTBDB 외부검증(3회)** — Open Access 라 자격심사 불필요하고 `.hea` 에 부위 라벨과 **Acute/Former 구분**이 있어 급성 MI 를 갈라 본다 → ④실험21 Chapman/Ningbo(MI 유무) → ⑤**🏁 실험23 표준 벤치마크 프로토콜 재현(6회) = 종착점** — fold 1–8 학습/9 검증/10 테스트로 `superdiagnostic` macro AUC + 부트스트랩 CI 를 내서 **직접 비교 가능한 단 하나의 숫자**를 확보한다. 낙폭이 크면 '지금까지의 유도 결론은 약한 백본 위에서의 상대 비교' 라고 한계에 명시한다. 종착점에 닿으면 **PTB-XL macro AUC · 외부 낙폭 · 감별질환 취약도** 세 숫자가 남고 그게 가장 인용 가치 있는 결과물이다. **구조적 확장 방향**(종착점 이후): 경색을 흉내내는 WPW·비후성심근증·LVH·RVH·좌각차단은 병적 Q파·QS·R파 진행 불량을 만들고 심낭염·조기재분극·PE 는 ST 를 올린다 — **임상 특이도를 무너뜨리는 것은 정상 심전도가 아니라 이들**이므로 최종 모델은 감별질환을 **같은 라벨 트리 안에서 함께 학습**해야 한다(2단계 라벨 공사에 포함). IPLMI(n=51·유병률 0.23%·시드SD 0.0430)는 **측정 불가**이므로 설계 근거로 쓰지 않는다. 실험1~18 종결 내역과 판단신호 미검정 5개 시험장은 각 결과 절과 ailab-2026-0018 참조",
   "status": "active",
   "sections": {

...
window.AILAB = {
 ...
 "cards": [
  ...
  },
  {
   ...
   "nextGoal": "실험1′·2·3·4·9·9b·10·10′·10″·13 종결. **실험13: 계측기가 검증됐다** — 이 퀘스트의 대전제(유도군 성능차 = 소견의 해부학적 평면)를 처음으로 '아는 답'과 대봤고 통과했다. 하벽경색(n=1247) 비중 174.5% vs 전중격경색(n=699) 20.4%, P-A Δ+1.610 [+1.080,+2.575], N0 무효 대조 통과. 실험10·10′·10″의 숫자가 무엇을 쟀는지 이제 안다. **그리고 더 나은 지표를 찾았다: 잔여 결손 `AUC({12}) − AUC({I,II})`** — 비율은 분모가 0에 가까우면 폭발/정의불가인데 하필 가장 강한 양성 사례(단일유도 포화)가 거기 걸린다(STTC 하벽허혈 `{II}` AUC 0.919인데 판정불가). 잔여 결손으로 재면 전두면 소견 0 이하(하벽경색 −0.025, 하벽허혈 +0.009), 횡단면 소견 양수(전벽허혈 +0.024, 전중격경색 +0.112)로 깨끗이 갈린다. **앵커가 생겨 LVH를 눈금 위에 놓을 수 있게 됐다 — +0.059 = 횡단면 앵커의 53%, 딱 중간이고 Cornell 해석이 예측하는 위치다**(증명은 아님, superclass가 달라 근사). 주의: 하벽 비중 174.5%는 `{12}` ⊃ `{I,II}` 라 정보론적으로 불가능하며 **클래스 내부 재배분**이다(12유도가 전중격을 0.733→0.874로 올리며 하벽을 희생) — '웨어러블이 12유도를 이긴다'로 인용 금지. 다음은 **실험13b(차별점 A 표를 잔여 결손 지표로 재작성 — 학습 0회, 포화 사례 판정 복구, 소견 10개 이상 확장)** 이고 그다음 **실험9c(샘플링 비율 스윕 + seed 3개 재현)**. 9c는 이제 macro만이 아니라 평면군별 잔여 결손도 함께 보고한다. 이어서 실험4c(RR 넣은 분류기의 SNR별 S 곡선) · 11(전극위치 augmentation) · 12(도메인 축 분리). 실험9b 결론: lead-agnostic의 저유도 붕괴는 균등 샘플링 부작용 — `{II}` 40% 샘플링으로 −0.0735 → −0.0235, 4구성 절대손실 합 절반. 실험10′ 결론: 혼합군(MI) 교호작용 +0.0859 확증. 실험3: 형태축−리듬축 분리 M−R +0.2862 확증. 실험4: 기권 신호는 클래스마다 달라야 한다(확률의 S 내부 AUC 0.184 역전). 판단신호 미검정 5개의 시험장은 ailab-2026-0018 참조",
   "status": "active",
   "sections": {

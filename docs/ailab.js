...
window.AILAB = {
 ...
 "cards": [
  ...
  },
  {
   ...
   "nextGoal": "실험1′·2·3·4·9·9b·10·10′·10″·13·13b·14·15 종결. **실험15: 첫 finding head 확증** — 소견(MI 부위)을 학습 타깃으로 올린 첫 실험이고 사전등록 3개 전부 통과, 붕괴 0건. ① 임상 동작점이 실제로 좋아졌다: IMI 민감도 0.90에서 경보율 47.3%(실험14 MI 전체) → **30.9%**. ② **차별점 A에서 \"이 모델 기준\" 단서를 뗄 수 있다** — 평면 D 격차가 13b의 0.061에서 0.148로 2.4배 커지며 부호 유지(13b 카드의 \"오배정 탓 과소추정\" 예측이 맞았다). ③ 다중라벨 전환이 측벽(90→489)·하측벽(0→478)을 되살렸다. 단서: P-1은 셋 중 가장 약한 검정이다(기준선 A는 부위를 구별하라고 배운 적이 없어 부위 전용 검출기가 이기는 게 설계상 보장) → 진짜 시험은 기준선 B인 **실험15c**. 새 관찰: LMI에서 {I,II}가 {12}보다 나아 사전배정 \"혼합\"보다 전두면 쪽으로 기울고, AMI(순수 전벽 0.099)가 ASMI(0.768)와 8배 차이며, IPLMI(1:426)가 가중 없이도 62배 lift로 살아남았다(단 {I,II}에서 0.007이라 살아있음≠쓸모있음). 다음은 **실험15c(기준선 B — 부위별 개별 이진 분류기 대조, 대표 2부위)** → **15b(pos_weight — 15에서 붕괴는 없었으나 LMI·AMI 동작점이 못 쓸 수준이라 여전히 후보)** → 유도 4구성 확장 → 실험9c(샘플링 비율 스윕 + seed 3개) · 4c. 2단계(라벨 트리 전체)는 15 배관이 검증됐으므로 확장이 쉬워졌다. IPMI(33)·PMI(17) 후벽은 탐지 대상 아님으로 확정 — V7–V9 없는 표준 12유도의 구조적 한계. 인프라: lib/medkos_run.py 를 Drive에서 repo로 가져와 버전관리 시작(테스트 포함), pipelines/ecg_preflight.py 로 라벨 어휘 검증·관문 3분 판정·붕괴 감시를 공용화. 판단신호 미검정 5개의 시험장은 ailab-2026-0018 참조",
   "status": "active",
   "sections": {

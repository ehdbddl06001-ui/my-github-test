...
window.AILAB = {
 ...
 "cards": [
  ...
  },
  {
   ...
   "nextGoal": "실험1′·2·3·4·9·9b·10·10′·10″·13·13b·14·15·15c·15d·16·17 종결. **실험17: 실체는 성공, 관문 설계가 실패했다.** 4전극 `{I,II,V1}` 최악 D **+0.0424**·중앙값 +0.0086, 5전극 `{I,II,V1,V5}` 최악 +0.0286·**중앙값 +0.0023**. 실험16 이 '**소견을 알고** 최선 2유도를 골라도 최악 0.065'였는데, 실험17 은 **소견을 몰라도 되는 고정 4전극 하나로 0.042** 를 냈다 — 3/7 부위는 12유도보다 낫다. 그런데 관문 5개가 전부 걸렸고 **원인은 데이터가 아니라 사전등록 설계 결함 셋**이다: ①**P-1 점추정 채점**(실험15d 3분 t-CI 규약 위반, 13b·14 에서 고쳤던 실수의 재발) — IMI 초과 +0.0108 은 시드 SD 0.0140 이내라 기각 성립 안 함. ②**P-2·P-3 문턱이 관측값 바로 위** — 점추정은 통과인데(ILMI 0.0424<0.05 · 경보배수 1.161<1.25) 3시드 t-CI 반폭이 걸친다. **시드 6개면 7부위 전부 확정**(추가 30회)이고, 이건 실험15c 와 정반대다(거기선 효과가 잡음 이하라 시드가 무의미했고, 여기선 효과가 명확하고 CI 만 넓다). ③**P-4·P-5 는 부위 평균이 효과를 지웠다** — V2 는 ASMI(n=2357, 시드SD 0.0012)에서 D 0.0295→0.0098·경보 0.274→0.222 로 뚜렷한데 n 이 7배 작은 AMI 와 동등가중 평균해 사라졌고, V5 는 LAT 를 [LMI,ALMI]로 잡아 **효과가 가장 큰 ILMI(+0.0424→+0.0004)를 빼먹었다**. **임상 발견**: `ILMI` 는 **II + V5 소견**이다 — {II} 0.9024 에서 I 를 더해도 V1 을 더해도 안 움직이다가 V5 하나에 0.9465(12유도 0.9469)로 뛰고 경보율도 0.278→0.168 로 12유도와 같아진다. 같은 측벽이라도 **고측벽 LMI 는 aVL(=I−II/2), 하후측벽 ILMI 는 V5** 로 갈린다. **용량 경쟁은 그대로**: IMI 는 `{I,II}`, LMI 는 `{I,II,V1}`, ILMI 는 `+V5` 가 최선이라 **어떤 단일 구성도 7부위 모두에 최적이 아니다**. 다만 5전극 기기는 신호를 이미 다 잡으므로 부위별 유도 사용은 **모델 내부 배선 문제**이고 추론 시 공짜다. 다음은 **실험18(4전극 사양 확정 — 실험16·17 arm 재사용 + 45회)**: A0 P-1 t-CI 재채점(학습 0회) · A1 부위별 유도 라우팅 천장을 **중첩 선택**으로 측정(학습 0회, 실험19 착수 근거) · P-1 6시드로 D<0.05 · P-2 경보배수<1.25 · P-3 **ASMI 단일 검정**으로 V2 · P-4 **ILMI 단일 검정**으로 V5 · P-5 `{I,II,V2,V5}` vs `{I,II,V1,V5}`. ⚠️ ILMI 는 6시드 상한이 0.0499 로 문턱 0.05 와 종이 한 장 차이라 **미결로 남을 수 있고 그것도 결과다** — 4전극에서 하후측벽만 경계이고 5전극에서 +0.0004 로 해소된다. 문턱은 사후에 옮기지 않고, 노트북이 관문 결과를 **사양 권고로 번역**한다. 그다음 15b(pos_weight + LMI 라벨 잡음) → 9c · 4c, 조건부로 실험19(평면 분리 헤드). 논문 전 필수는 **외부검증(Chapman/Ningbo)·시드 반복(15d·18 로 착수)·급성 코호트(MIMIC-IV-ECG)** 셋. 판단신호 미검정 5개의 시험장은 ailab-2026-0018 참조",
   "status": "active",
   "sections": {

...
window.AILAB = {
 ...
 "cards": [
  ...
  },
  {
   ...
   "nextGoal": "실험1′·2·3·4·9·9b·10·10′·10″·13·13b·14·15·15c·15d·16 종결. **실험16: 사전등록 4개 전부 지지 — 차별점 A 가 '설명'에서 '전극 배치 규칙'으로 올라갔다.** 효과가 시드 잡음의 **12.9~16.3배**라 견고하다. **P-4 교차 +0.1589 [+0.1346,+0.1832]**: 유도 수가 같을 때 `W = AUROC({II,V1}) − AUROC({I,II})` 의 **부호가 소견을 해부학적으로 갈라놓는다**(LMI −0.225 · ALMI −0.107 · IMI −0.067 ‖ ASMI +0.062 · IPLMI +0.068 · AMI +0.121). **P-3 V1 선택성 +0.1361**: V1 은 아무 데나 듣는 게 아니라 횡단면에만 선택적으로 듣는다(AMI +0.193 · ASMI +0.136 vs IMI +0.028). **P-2**: IMI `D({I,II})` = −0.0022 [−0.0134,+0.0090] → **하벽은 사지 2유도가 12유도와 동등**. 가장 극적인 자리는 LMI — `{II}` 단독 0.578(동전 던지기)이 `{I,II}` 에서 **0.856**으로, 유도 하나에 +0.28 이 뛴다. **`aVL = I − II/2`** 라 I·II 만 있으면 aVL 이 계산으로 나오기 때문이고, 대신 V1 을 붙이면(0.632) 거의 안 오른다. **P-1 위반 1건이 더 큰 발견이다**: ILMI 는 V1 을 더하니 D 가 +0.044→+0.058 로 악화되고, LMI(0.856 vs 0.833)·IMI(0.922 vs 0.919)에서 **사지 2유도가 12유도를 이긴다** → **12유도 모델이 전두면 소견에서 성능을 흘린다**(흉부 6유도가 용량을 가져가 aVL 성분 희석). 실험9b 의 거울상이고 실험18(유도 attention·평면 분리)로 큐잉했다. **배포 교훈**: 소견별 최선 2유도의 잔여 결손은 최대 0.065·중앙값 0.042 로 작은데 **경보율은 1.9배**가 된다(ASMI 19.8%→36.8%) — 민감도 0.90 은 꼬리라 작은 AUROC 차가 거기서 크게 벌어진다 → **앞으로 배포 관문은 AUROC 가 아니라 경보율로 건다**. IMI 경보율은 실험14 47.3% → 실험15 30.9% → **실험16 `{I,II}` 28.6%** 로 **유도를 줄이면서** 개선됐다. **정정 2건**: ①'전극 수가 같다'는 틀렸다 — `{I,II}`=3전극, `{II,V1}`=4전극(V1 은 Wilson 중심단자 필요). 교차 결과는 유효하나 배포 권고는 이를 반영한다. ②**실험15d 의 G0(GPU 비결정성) 철회** — G0-b 가 `{I,II}`·`{12}` × 7부위 전량 OOF 에서 **ΔAUROC = 0.0000 (14/14)** 를 냈다. **학습은 결정론적**이고, 따라서 15d 의 시드 SD 는 **순수 시드 분산**으로 확정된다(15d 헤드라인은 강화된다) + **저장 arm 은 세션을 넘어 재사용 가능**하다. 다음은 **실험17(4전극 웨어러블 — `{I,II,V1}`·`{I,II,V2}`·`{I,II,V1,V5}` 추가, 실험16 arm 재사용으로 45회 ~45분)**. 핵심 질문은 **소견을 모른 채 전극을 붙여야 하는 배포 상황**에서 두 평면을 한꺼번에 덮으면 12유도에 얼마나 붙는가다. 관문은 `D < 0.05`(전 부위)와 **경보율 ≤ 12유도의 1.25배**. 합집합 P-1 이 실패하면 답은 '유도를 더 붙여라'가 아니라 **'구조를 바꿔라'(실험18)** 다. 그다음 15b(pos_weight + LMI 라벨 잡음 점검) → 9c · 4c. 논문 전 필수는 **외부검증(Chapman/Ningbo)·시드 반복(15d 로 착수)·급성 코호트(MIMIC-IV-ECG)** 셋. 판단신호 미검정 5개의 시험장은 ailab-2026-0018 참조",
   "status": "active",
   "sections": {

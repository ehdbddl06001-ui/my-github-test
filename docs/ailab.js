...
window.AILAB = {
 ...
 "cards": [
  ...
  },
  {
   ...
   "nextGoal": "실험1′·2·3·4·9·9b·10·10′·10″·13·13b·14·15·15c·15d·16·17·18 종결. **실험18 CI 채점 반영 — 점추정 판단 2건을 정정한다.** ①**'승자는 {I,II,V2,V5}' 는 확립되지 않았다**: P-5 −0.0146 [−0.0704,+0.0412], 효과/시드잡음 0.65배로 규약 ②에 따라 **종결**. `최악 D`(max)를 집계 통계로 쓴 것이 설계 실수였다 — 최댓값은 가장 시끄러운 부위(IPLMI n=51, SD 0.0430)가 지배하고, 시드별 V1+V5 최악이 +0.0248/**+0.0740**/+0.0238 로 한 시드만 튀어 차이를 다 만들었다. 중앙값으로는 V1+V5 +0.0023 vs V2+V5 +0.0030 으로 **사실상 같다**. 앞으로 구성 간 비교는 **중앙값 또는 부위별**로 한다. ②경보배수는 1.30 이 아니라 **1.218** [1.120,1.316](시드별 중앙값의 평균 — 내가 평균의 중앙값을 냈었다). **확립된 것**: **P-3 ✅ 전벽 커버 유도는 V2 다** — ASMI 단일 검정 −0.0198 [−0.0308,−0.0088], 잡음의 **4.48배**. P-1 은 5/7 확정(IPLMI·ALMI 미결, 최악 ALMI +0.0361) · P-2 미결 · P-4(ILMI +V5) −0.0419 [−0.0861,+0.0023] 2.36배로 **4시드면 확정** · A0 미결로 실험17 P-1 기각 철회 확정. **A1 라우팅 결론은 앙상블 교란으로 철회**하되 **앙상블 이득 +0.0256(중앙값)** 은 남는다. **민감도 판단(핵심)**: 민감도 0.90 PPV 는 ASMI 45.7%·IMI 35.3% 둘만 쓸 만하고 AMI 3.2%·LMI 1.8%·IPLMI 0.4% 이며 **12유도도 똑같다** → **유도 최적화는 천장에 닿았고 남은 병목은 유병률 × 동작점**이다. **🔓 외부검증 경로를 바꿨다 — MIMIC-IV-ECG 없이 간다.** MIMIC 은 credentialed 라 추천인 서명이 필요해 개인 과제로는 문턱이 높다. 대체재는 **PTBDB(원조 PTB Diagnostic ECG Database, Open Access)**: 549레코드/290명·**MI 148명**·15유도 1000Hz. 결정적으로 ①`.hea` 에 **`Acute infarction (localization)`** 형식의 **부위 라벨이 있고** 문헌 분류(AMI·ASMI·ALMI·IMI·ILMI)가 우리 7부위와 겹치며 ②**MI 유병률 51%** 라 PPV 를 다른 유병률에서 보고 ③**`Acute` 와 `Former` 를 따로 기록**해 **PTB-XL 이 못 다룬 급성 MI 를 갈라 볼 수 있다**(MIMIC 이 필요했던 진짜 이유가 이것이다). 규모·도메인 이동은 **Chapman-Shaoxing+Ningbo(45,152레코드, Open Access)** 로 하되 SNOMED 라벨이라 **MI 유무 수준**이다. PPV 자체는 베이즈 함수라 **PPV–유병률 곡선**으로 새 데이터 없이도 정직하게 보고된다. 다음은 **실험19(동작점 2단계 재설계, 30회 ~50분)** → **실험20(PTBDB 외부검증 — 학습 3회)** → 21(Chapman/Ningbo) → 15b → 9c·4c, 조건부로 22(평면 분리 헤드). 판단신호 미검정 5개의 시험장은 ailab-2026-0018 참조",
   "status": "active",
   "sections": {

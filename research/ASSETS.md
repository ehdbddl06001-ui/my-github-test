...
| data-mitdb-cache | annotation/header cache | `MyDrive/mitbih/raw_ann/mitdb/` (folder id `151DJAcjCbDXCoy9ZIPudbtSuVziG1fnj`) | provenance 참조 · Q5-A symbol 복구 | verified (2026-08-09) | `.hea` + `.atr`, `.dat` 없음 | 360 Hz · 2 leads; 이 폴더만으로 waveform 재구축 불가. **경로 정정**: 이전 표기 `MyDrive/mitbih/mitdb/` 는 틀렸다 — 실제 부모는 `raw_ann`(folder id `1jJSQbAcaVzA1cw2vIL0h3A4cOT-PR-lb`). Q5-A가 `t`↔`.atr` sample 조인으로 원 symbol(A/a/J/S)을 복구하는 유일한 경로 |
| baseline-v10-pwave-run | model run (legacy ablation) | `MyDrive/mitbih/ablation_step9d/pwave/ens.npz` | `mit-bih/colab_step9d_final.py :: run_final("pwave")` · Q5-A primary baseline | verified (2026-08-09, 산출물 존재) | keys `prob`(n,3)·`y`·`pid`; seeds 1000–1004 앙상블 평균 | DS2만 채점 · annotation index 없음 → Q5-A가 `mamba_data.npz`(SHA `b1c16106…`)와 `pid`·`y` 전량 대조로 행 대응을 검증한 뒤 `t`로 키 부여. per-seed 미저장 → seed variability 산출 불가 |
| baseline-base26-control | model run (legacy ablation) | `MyDrive/mitbih/ablation_step9d/base26/ens.npz` | `colab_step9d_final.py :: run_final("base26")` · Q5-A paired control | verified (2026-08-09) | 동일 스크립트·seed·저울 | V10과 **P파 특징 블록만** 다른 짝 대조군(`use_pw = tag=="pwave"`). 이름이 같은 `ablation_step11/base26`·`ablation_step13/base26`과 혼동 금지 — Q5-A는 primary와 같은 parent run으로만 범위를 좁힌다 |
...
| run-q5a-inventory | analysis input index | `MyDrive/MedKOS/ecg-model/runs/<ts>_EXP-2026-004_q5a_inventory/` | `notebooks/quest50_q5a_patient_failure_atlas.ipynb` (mode `INVENTORY`) | **planned — NOT RUN** | — | Q5-A가 만들 후보 run 표(`source_inventory.json/csv`)와 `baseline_freeze.json`. 읽기 전용 스캔이며 기존 폴더를 수정하지 않는다 |
| run-q5a-atlas | derived analysis (no training) | `MyDrive/MedKOS/ecg-model/runs/<ts>_EXP-2026-004_q5a_patient_failure_atlas/` | `mit-bih/q5a_patient_failure_atlas.py` · `notebooks/quest50_q5a_patient_failure_atlas.ipynb` | **planned — NOT RUN** | — | 환자·subtype·RR·atrial proxy·quality 실패 지도 + 사전등록 branch 판정. 상태는 `MEASURED` / `BLOCKED_MEASURED` / `RESULT_NOT_RUN` 중 하나. 원본 bundle fingerprint 전후 비교로 불변 검증 |
| baseline-v9-kink-noctx | model run (historical) | **존재하지 않음** | 과거 V9 실험 | **ARTIFACT_ABSENT (2026-08-09 실측)** | 기록값 S PR-AUC 약 0.597 (`PROJECT_STATE.md`) — **검증 불가** | Drive 전량 조회(run/tag 폴더명·파일명·색인 본문)와 repo 전량 grep에서 `kink`/`noctx` 부재. 로컬 PC 보관 또는 미저장으로 추정. 한계: Drive 색인은 `.npz` 내부를 읽지 않으므로 다른 이름의 V9까지 배제하지는 못함. **Q5-A를 위해 재학습하지 않는다** |

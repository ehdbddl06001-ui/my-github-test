...
| baseline-v9-kink-noctx | model run (historical) | 미확정 — Q5-A `INVENTORY`가 provenance로 특정 | 과거 V9 실험 | **unverified: run 미특정** | 기록값 S PR-AUC 약 0.597 (`PROJECT_STATE.md`) | metric 단위(beat-micro vs record-macro) 미확정. 후보가 여럿이면 `AMBIGUOUS_BASELINE`으로 중단. Q5-A를 위해 재학습하지 않는다 |
| baseline-v10-pwave | model run (historical) | 미확정 — Q5-A `INVENTORY`가 provenance로 특정 | 과거 V10 실험 | **unverified: run 미특정** | 기록값 S PR-AUC 약 0.660 (`PROJECT_STATE.md`) | 저장 probabilities·안정 beat key가 없으면 aggregate metric만 기록하고 beat-level 비교에서 제외 |

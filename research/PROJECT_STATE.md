...
- 다음: **Q5-B-0b 제안**(승인 대기) — 동점 후보들이 **같은 symbol을 가지면** 어느
  것을 골라도 답이 같으므로 부여한다(추측이 아니라 추론). 새 gate 2개
  (`tie_set_symbol_agreement`, `subtype_distribution_agreement` — ecg_multi가
  record별 참 A/a/J/S 개수를 알고 있으므로 편향을 직접 잡는다)와 **같은 규칙으로
  다시 계산한 wrong-record 영가설**을 함께 등록한다. 통과 못 하면 그때 영구 종결.

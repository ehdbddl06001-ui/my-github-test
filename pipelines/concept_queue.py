...
def build(events: list[dict]) -> dict:
    ...
    return {"generated": ll.kst_day(""), "note": notes, "link": link_list,
            "counts": {"note": len(notes), "link": len(link_list),
...
def main(argv: list[str]) -> int:
    ...
    note, link = q["note"], q["link"]
    if a.limit:
        note, link = note[:a.limit], link[:a.limit]
    if a.json:
        print(json.dumps({"note": note, "link": link, "counts": q["counts"]}, ensure_ascii=False, indent=1))
        return 0
    if not events:

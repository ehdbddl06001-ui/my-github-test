...
- weekly_topic(): ISO 주차로 커리큘럼을 순환 선택(결정론적, 임시 컨테이너에서도 동일).
...
사용:
  python pipelines/datasets.py                 # 이번 주 실습 주제 + 카탈로그 요약
  python pipelines/datasets.py --modality ecg  # 특정 양식 데이터셋만
  python -c "from pipelines.datasets import weekly_topic; print(weekly_topic())"
"""
...
def iso_week(when: date | None = None) -> int:
    return (when or date.today()).isocalendar().week


def weekly_topic(when: date | None = None) -> dict:
    """ISO 주차로 커리큘럼을 순환 선택. 같은 주면 어느 컨테이너에서 돌든 동일 결과."""
    week = iso_week(when)
    item = CURRICULUM[week % len(CURRICULUM)]
...
def main() -> int:
    ap = argparse.ArgumentParser(description="MedKOS 오픈 의료 데이터셋 레지스트리")
    ap.add_argument("--modality", help="양식 필터(ecg/brain-mri/chest-xray/ct/path/derm/fundus/tabular)")
    args = ap.parse_args()

    wt = weekly_topic()
    print(f"── 이번 주(ISO {wt['week']}주차) 실습 주제 ─────────────────────")
    print(f"  🎯 {wt['goal']}")
    print(f"  🧠 모델: {wt['arch']}")

...
def unit_label(meta: dict) -> str:
    ...
    구한다(시간표가 유일 기준 — 파일명·수업계획서 날짜를 쓰지 않는다). 둘 다 없으면
    `subregion` 슬러그를 부위 기준 배정(`session_for_region`)에 태운다 — 과거 학기
    인제스트 카드가 여기 해당한다.
    ...
    if meta.get("session_no"):
        ...
        for raw in meta.get("scheduled_dates", []) or []:
            ...
            if no and no not in sessions:
                sessions.append(no)
        sessions.sort()
        if not sessions and meta.get("subregion"):
            no = session_for_region(str(meta["subregion"]))
            if no:
                sessions = [no]

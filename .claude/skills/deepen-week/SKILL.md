...
## 무엇을 읽나(정직한 출발점 — 예측 금지)
작업지시서가 가리키는 **실제 파일을 연다**. 순서가 중요하다:
1. **`notebook`(실측)** — 작업지시서의 `notebook_is_real: true`면 이건 사용자가 **실제로 돌린**
   노트북이다. **셀 단위로** 읽어 분할·정규화·모델·학습·평가를 사실대로 파악한다. 여기서
   본 것이 A·B·D의 근거다. (`notebook_is_real: false`면 실행 로그가 없다는 뜻 → glob 추정이라
   예측 위험. A절에 "추정"임을 명시하고, 사용자에게 `ingest_run.py`로 실제 노트북 등록을 권한다.)

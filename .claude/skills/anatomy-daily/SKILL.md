...
- **카데바 위 글자는 덮지 말고 복원한다(2026-08-19 사용자 지적 — 매우 중요)**:
  ...
  - 조각 밑이 **표본(밝은 조직)** → `label_boxes` 로 보내 **주변 조직 + 앞뒤 프레임**
    으로 복원한다(donor → 점진 인페인팅 → 질감 매칭).
  - 조각 밑이 **검은 여백** → 그때만 `black_boxes`(덮어도 잃을 게 없다).
  - 한 라벨이 조직에서 시작해 여백으로 넘어가는 일이 흔하므로 **세로로 쪼개** 구간별로
    정한다. 26px 미만 조각은 이음매가 보여 이웃에 합친다.
  - `--build` 는 앞뒤 3장 중 **같은 장면**(축소 그레이 평균차 < 12)을 donor 로 자동 고르고,
    donor 에도 같은 자막이 떠 있으므로 donor 쪽 글자 자리는 `donor_bad_boxes` 로 뺀다.
  - 핀이 가리키는 자리가 **검은 박스** 안이면 그 페이지는 버린다. 복원되는 자리는
    조직이 남으므로 버리지 않는다.
...
- **예습시험 모드(수업일 연동 — 최우선)**: 예습시험은 **수업당 10문제**,
  그날 배울 **회차 범위에서만** 출제된다. 회차 매핑은 **부위 기준**이다 —
  자세한 규칙은 아래 4d.
...
- **교수명·파일명 날짜로 회차를 정하지 않는다.** 그 자료가 다루는 **부위**를 2026
  시간표(`SCHEDULE_2026`)의 실습주제에 맞춘다.
- 결정론은 `anatomy_schedule.session_for_region("<부위/영상 타이틀>")` → 회차 번호.
  못 찾으면 `None` — **추측하지 말고** 사용자에게 묻거나 카드에 근거를 남긴다.
- 한 스캔 묶음이 **여러 회차에 걸칠 수 있다.** 과거 실습표의 한 회차가 2026에서는
  둘로 쪼개져 있을 수 있으므로 **페이지 단위로 부위를 보고 나눈다.**
  실측 예: 과거 '실습6'(문용석pf) 스캔 178장 중 목·넓적다리 앞/안쪽·종아리 앞·발등은
  2026 **6회차**, 그 안에 섞인 발목 안쪽면·굽힘근지지띠는 2026 **7회차**다.
- 카드 `session_no`·`scheduled_dates` 는 이렇게 정한 **2026 회차**를 쓰고,
  `source` 에는 원본이 과거 학기임을 적는다(교수명은 출처 표기용일 뿐 배정 근거가 아니다).
...
- **회차 스캔은 전수 처리한다(2026-08-30 사용자 지시 — 이전의 '하루 2개 선별'을 대체)**:
  ...
  회귀는 `test_every_label_becomes_a_question`.
  · 한 장에 라벨이 `MAX_PINS_ALL`(20)을 넘으면 그 페이지는 버린다 — 핀이 겹쳐 못 푼다.
  · 사람 QA(`--sheet` 대지 → `--reject`)는 그대로 유지한다. 전수로 만들수록 검출기가
    놓친 흐린 캡션이 섞일 확률이 올라가므로 오히려 더 중요하다.
  · 하루 처리량이 부담되면 **페이지를 나눠서** 밀린 만큼 다음 실행이 이어받는다
    (3단계 catch-up). 라벨을 골라내서 줄이지는 않는다 — 줄이는 축은 '페이지 수'이지
    '한 페이지 안에서 물어볼 라벨 수'가 아니다.
...
- **회차 산출물은 서브노트 한 파일로 합친다(기본)**: 도해 + 표 + **실사 태깅 문항·정답**까지
  ...
  **암기 3종 세트는 자동으로 붙는다**(본문 뒤·문항 앞):
  ① 두문자·대조표 ← frontmatter `mnemonics:` (사람이 쓴다. 5줄 이상)
  ② 빈칸 채우기 ← 본문의 `==하이라이트==` 를 그대로 `( ① )` 로 바꾼 것 + 정답 목록
  ③ 30초 자가 점검 ← `### 소제목` 과 콜아웃 라벨을 '덮고 말하기' 체크리스트로
  → ②③은 **본문에서 파생**되므로 지어내지 말고, 외울 것을 본문에서 `==...==` 로
  표시하는 데 집중한다. 표 안의 하이라이트도 '첫 칸 — 나머지' 한 문장으로 자동 변환된다.
...
- **매일 전달 — 이것이 일일 루틴의 마무리 단계다**:
  ...
  3. **문항 텍스트본을 만들어 함께 보낸다**(사용자 지시 2026-08-30 — "만든 문항들은
     드라이브에 저장할 수 있도록"). 그림을 뺀 글만이라 Docs 로 바로 올라가고 비용이 없다:
     ```
     python pipelines/anatomy_export_questions.py --session NN
     ```
     → `.private/anatomy/export/sNN-questions.md`. `SendUserFile` 로 서브노트 PDF와
     함께 보낸다(사용자가 Drive 에 보관). 서브노트 PDF 는 **그림**, 이 파일은 **글**이라
     둘이 짝이다 — 검색·복습은 이 텍스트본에서 된다.
  4. Google Drive MCP를 **쓸 수 있으면** 텍스트 동반본(서브노트 카드 본문 + 위 문항
     텍스트본)을 `MedKOS-해부-복원자료`(id `1W2AYQSr-zzKseja7ppukc1uLMh6CgOGl`)에
     Docs로 올린다. MCP가 없으면(루틴 컨테이너에는 보통 없다) 이 단계만 건너뛰고
     보고에 한 줄 남긴다 — 3번의 파일 전송이 있으므로 전달은 이미 끝나 있다.

...
페이지 선정 → 필기 색 검출 + 검정 손글씨 bbox 지정(시각 판단) → inpaint 복원
...
- **페이지 선정 기준(3Q 개요 실측)**: 땡시는 1차 30문항(카데바 5구)·2차
  ...
  번호 항목과 겹치는 구조물 최우선.
- **페이스**: 실사 문항은 **하루 2개**(쉬움 1 + 어려움 1 권장), 땡시 D-14부터
  **하루 4개**로 램프업. 근거: 해부는 2학점(개요: "해부에 힘 빼지 말 것"),
  총 66문항 대비 실사 문항 100~150개면 충분 — 20개 파일을 전부 처리하지
  말고 태깅 후보만 선별한다. 밀리면 3단계 catch-up이 흡수.

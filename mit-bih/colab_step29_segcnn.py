# =============================================================================
#  colab_step29_segcnn.py  —  [STEP 29] T/ST 개인편차(segdev)를 CNN에
#
#  step28 단일스캔: best 위에서 T/재분극 개인편차가 최대 증분 —
#    ST상관 +0.071, 분절총편차 +0.062, QRS-T시작편차 +0.028, T지연 +0.024 (전부 같은 방향=신뢰).
#  P계열은 약함(사장님 관찰: T주변에 신호). repol 승자(+0.018~0.037)보다도 큰 증분 → CNN 확인.
#  정밀도(오경보)가 목표라 PR-AUC + best-F1(PREC/SEN/F1) 둘 다 보고.
#
#  채택 4개: ST상관(9)·분절총편차(10)·QRS-T시작편차(8)·T지연(4)  (부호일관 T/ST 클러스터)
#  선행 로드: step12(_WST)·step15(extract_morpho)·step18(extract_repol)·step28(extract_segdev/_SEGDEV)
#  실행:
#    exec(open('/content/drive/MyDrive/mitbih/colab_step29_segcnn.py').read())
#    run_seg(Kwst=40)
# =============================================================================
import os
import numpy as np
...
_SAVE="/content/drive/MyDrive/mitbih/ablation_step29"; _C={}
_REPOLK_IDX=[0,1,4,5]
_SEG_IDX=[9,10,8,4]      # ST상관, 분절총편차, QRS-T시작편차, T지연
...
def run_seg(Kwst=40, seeds=None):
    import torch, torch.nn as nn
    ...
    Fs =np.concatenate([Fb,Sk],1).astype("float32")                                 # best+segT
    print(f"best dim={Fb.shape[1]}  /  best+segT dim={Fs.shape[1]}  (추가4: ST상관·분절총편차·QRS-T편차·T지연)")
    ...
    R={"b":[],"s":[]}
    for seed in seeds:
        R["b"].append(train_one(Fb,seed)); R["s"].append(train_one(Fs,seed))
        print(f"  seed {seed}:  best S={met(R['b'][-1],y2)[0]:.4f}   +segT S={met(R['s'][-1],y2)[0]:.4f}")
    os.makedirs(_SAVE,exist_ok=True)
    print(f"\n{'='*56}")
    for tag,nm in [("b","best      "),("s","best+segT ")]:
    ...
    print(f"\n▶ +segT가 S·특히 PREC/F1 올리면 = T/ST 개인편차가 정밀도 보강(사장님 T관찰 성립).")
    return R

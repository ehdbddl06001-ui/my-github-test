# =============================================================================
#  colab_step39_precision.py  —  [STEP 39] 정밀도 겨냥 비-앙상블 (DTW S-N판별 등) CNN
#
#  라우팅/스태킹 결합은 막다른 길(과적합). 정밀도(오경보=N↔S경계)를 '단일모델 특징'으로 직공:
#   · DTW S-N판별 = 시간불변 매치드필터 판별자(N템플릿 vs S템플릿, 조기성 허용) → 오경보 기각
#   · DTW 전체(조기성 워핑) / segT+VCG+DTW_SN 결합 = 정밀도 아이디어 총동원
#  best(26+WST+morph+repol)에 얹어 15seed 트림 + PREC/SEN/F1 비교 (정밀도 오르나).
#
#  선행 캐시: _WST, extract_morpho/_MORPHO, extract_repol/_REPOL, extract_segdev/_SEGDEV,
#            extract_vcg/_VCG, extract_dtw/_DTW(15특징 v2)
#  실행:
#    exec(open('/content/drive/MyDrive/mitbih/colab_step39_precision.py').read())
#    run_prec(Kwst=40)                 # 느리면 seeds=list(range(1000,1008))
# =============================================================================
import os
import numpy as np
...
_SAVE="/content/drive/MyDrive/mitbih/ablation_step39"; _C={}
_REPOLK_IDX=[0,1,4,5]; _SEG_IDX=[9,10,8,4]; _DTW_SN_IDX=[2,5,8,11,14]   # DTW S-N판별(구간별)
...
def run_prec(Kwst=40, seeds=None):
    import torch, torch.nn as nn
    ...
    Xs=globals().get("_SEGDEV"); Xv=globals().get("_VCG"); Xd=globals().get("_DTW")
    ...
    dsn=Xd[:,_DTW_SN_IDX]; seg=Xs[:,_SEG_IDX]
    CFG={"best":Fbest,
         "best+DTW_SN":np.concatenate([Fbest,dsn],1).astype("float32"),
         "best+DTW_full":np.concatenate([Fbest,Xd],1).astype("float32"),
         "best+segT+VCG+DTW_SN":np.concatenate([Fbest,seg,Xv,dsn],1).astype("float32")}
    ...
    for name,F in CFG.items():
        ...
        print(f"  {name:24s} 트림 S={S:.4f}  PREC={pr:.3f} SEN={se:.3f} F1={f1:.3f}")
    b=res["best"]
    print(f"\n{'='*56}\n▶ best 대비:")
    for name,(S,pr,se,f1) in res.items():
        if name=="best": continue
        print(f"  {name:24s}: ΔS={S-b[0]:+.4f}  ΔPREC={pr-b[1]:+.3f}  ΔF1={f1-b[3]:+.3f}")
    print(f"\n  ★ ΔPREC↑ 이면 = DTW S-N판별이 오경보 기각(정밀도 병목 완화). Farag PREC 0.827 기준.")
    return res

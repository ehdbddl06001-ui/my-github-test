# =============================================================================
#  colab_step1.py  —  [STEP 1] 목적함수 정비 · Colab + Google Drive 저장판
#
#  사용법(요약):
#    1) 이 파일 내용을 Colab '라이브러리 셀'에 통째로 붙여넣고 1번 실행(정의만 됨).
#    2) DATA_PATH 를 Drive에 올려둔 mamba_data.npz 경로로 지정.
#    3) run_ablation("A"|"B"|"C"|"D") 를 각각 실행 → 결과가 Drive SAVE_DIR 에 저장됨.
#
#  아키텍처는 원본 cnn_base_psa2 와 100% 동일. 오직 손실함수/샘플링만 스위치로 추가.
# =============================================================================
import os, json
import numpy as np
import torch
...
# ─────────────────────────────────────────────────────────────────────────────
#  경로 설정 (Colab에서 이 두 줄만 본인 환경에 맞게 수정)
# ─────────────────────────────────────────────────────────────────────────────
DATA_PATH = "/content/drive/MyDrive/mitbih/mamba_data.npz"        # 업로드해둔 데이터
SAVE_DIR  = "/content/drive/MyDrive/mitbih/ablation_step1"        # 결과 저장 폴더
...
SEEDS=[1000,1001,1002]     # 최종 논문용은 list(range(1000,1015))
...
class Cfg:
    ...
    # ---- [STEP 1] 목적함수 스위치 ----
    loss_type="focal"        # "wce" | "focal"
    focal_gamma=2.0
    supcon_weight=0.2        # 0.0이면 SupCon off
    supcon_temp=0.1
    balanced_sampler=True


# ─────────────────────────────────────────────────────────────────────────────
#  모델 (원본 구조 + SupCon용 proj_head)
# ─────────────────────────────────────────────────────────────────────────────
...
class CNNBasePSA2(nn.Module):
    def __init__(self,cfg):
        ...
        self.proj_head=nn.Sequential(                          # SupCon 전용
...
class FocalLoss(nn.Module):
    ...
    def forward(self,logits,y):
        ...
        logpt=logp.gather(1,y.view(-1,1)).squeeze(1); pt=logpt.exp()
...
class SupConLoss(nn.Module):
    ...
    def forward(self,feats,labels):
        ...
        labels=labels.view(-1,1); pos=(labels==labels.t()).float()
        self_mask=1-torch.eye(B,device=device); pos=pos*self_mask
        exp=torch.exp(sim)*self_mask
        logp=sim-torch.log(exp.sum(1,keepdim=True)+1e-12)
        cnt=pos.sum(1); mean_logp_pos=(pos*logp).sum(1)/cnt.clamp(min=1)
        valid=cnt>0
...
def _loader(arrs,batch,shuffle,balanced=False):
    ...
    if balanced and shuffle:
        ...
        w=1.0/np.clip(cnt,1,None); samp_w=torch.from_numpy(w[yv]).double()
...
def train(model,tr_loader,va_loader,cfg,device):
    ...
    use_sc=cfg.supcon_weight>0; best=-1; best_state=None
...
def run_one_seed(cfg,seed,ds1,ds2,device):
    ...
    model=train(model,_loader(tr,cfg.batch,True,balanced=cfg.balanced_sampler),
...
# ─────────────────────────────────────────────────────────────────────────────
PRESETS={
    "A": dict(loss_type="wce",   supcon_weight=0.0, balanced_sampler=False),  # 원본 재현
    "B": dict(loss_type="focal", supcon_weight=0.0, balanced_sampler=False),  # +Focal
    "C": dict(loss_type="wce",   supcon_weight=0.2, balanced_sampler=True),   # +SupCon+샘플러
    "D": dict(loss_type="focal", supcon_weight=0.2, balanced_sampler=True),   # 전부
}

def run_ablation(tag, seeds=None, **override):
    """tag in {'A','B','C','D'}. 결과를 Drive SAVE_DIR/tag/ 에 저장."""
    ...
    print(f"===== RUN {tag} =====  loss={cfg.loss_type} gamma={cfg.focal_gamma} "
          f"supcon_w={cfg.supcon_weight} balanced={cfg.balanced_sampler}  device={device}")
    ...
    print(f"DS1 N={len(ds1[3])}  DS2 N={len(ds2[3])}")
    rows=[]
    for seed in seeds:
        print(f"  [seed {seed}]")
    ...
    summary={"tag":tag,"config":{k:getattr(cfg,k) for k in
             ["loss_type","focal_gamma","supcon_weight","supcon_temp","balanced_sampler","class_w","epochs","batch","lr"]},
             "seeds":list(seeds),"per_seed":rows,"aggregate":agg}
    with open(os.path.join(out,f"summary_{tag}.json"),"w") as fp: json.dump(summary,fp,indent=2)
    print(f"\n----- RUN {tag} 집계 (원본 목표 S 0.613 / V 0.932) -----")
    for k in ["S_PR_AUC","V_PR_AUC","macro_PR_AUC","S_Se","S_P","S_F1"]:
        print(f"  {k:12s} = {agg[k]['mean']:.4f} ± {agg[k]['std']:.4f}")
    print(f"\n저장됨: {out}/  (npz {len(seeds)}개 + summary_{tag}.json)")
    return summary

# =============================================================================
#  colab_step40_weight.py  —  [STEP 40] S 가중치 스윕 ("S 과집중" 가설 검증)
#
#  STEP39: best+DTW_full = 새 최고(S 0.720 / PREC 0.699 / F1 0.736). 정밀도 +0.048.
#  사장님 통찰: SEN↑ PREC↓ = 오경보 = 'S 과예측'. 손실이 S를 3배 upweight(1,3,1.5) → 원인일 수 있음.
#  → best+DTW_full 위에서 S 가중치 스윕. 낮추면 PREC↑/PR-AUC↑ 이면 = 정말 S에 과집중했던 것.
#
#  선행 캐시: _WST, _MORPHO, _REPOL, _DTW(15특징 v2)
#  실행:
#    exec(open('/content/drive/MyDrive/mitbih/colab_step40_weight.py').read())
#    run_wsweep()               # 느리면 seeds=range(1000,1008)
# =============================================================================
import os
import numpy as np
...
_SAVE="/content/drive/MyDrive/mitbih/ablation_step40"; _C={}; _REPOLK_IDX=[0,1,4,5]
...
def run_wsweep(Kwst=40, Sweights=(1.5,2.0,3.0,4.0), seeds=None):
    import torch, torch.nn as nn
    ...
    print(f"base=best+DTW_full dim={F.shape[1]}, S가중치 스윕 {Sweights}")
    ...
    for Sw in Sweights:
        Pn=np.stack([train_one(s,Sw) for s in seeds],0); np.savez(os.path.join(_SAVE,f"Sw{Sw}.npz"),P=Pn,y=y2)
        Pt=trim(Pn); S,V=met(Pt,y2); pr,se,f1=_f1(y2,Pt[:,1]); res[Sw]=(S,pr,se,f1)
        print(f"  S가중치={Sw:<4} 트림 S={S:.4f}  PREC={pr:.3f} SEN={se:.3f} F1={f1:.3f}")
    print(f"\n{'='*54}\n▶ S가중치 낮출수록 PREC·PR-AUC↑ 면 = 'S 과집중'이 정밀도 병목이었음(사장님 가설 성립).")
    print(f"  최고 F1 지점 채택 → 최종 모델.  (현재까지 best+DTW_full@Sw3.0 = F1 0.736)")
    return res

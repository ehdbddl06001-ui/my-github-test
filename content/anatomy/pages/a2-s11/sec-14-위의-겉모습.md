---
id: anatomy-pg-a2-s11-위의-겉모습
type: anatomy
kind: source_page
topic: Anatomy
subtopic: 위의 겉모습
date: '2026-08-12'
confidence: medium
source: 11차시(0922) 허미선pf.pdf〔과거 학기 파일명 — 2026 담당·회차 근거 아님〕
source_id: a2-s11
source_file_id: 1vTTFerg-qAFnYV0ofiolfcj5RDefrwml
source_file_name: 11차시(0922) 허미선pf.pdf〔과거 학기 파일명 — 2026 담당·회차 근거 아님〕
source_page: null
region: abdomen
classification_confidence: 1.0
structure_classes: []
layers: [cavity-visceral]
scheduled_dates: []
exam_phase: tagging-2
terms:
- {ko: 들문부분, en: cardial part}
- {ko: 위바닥, en: fundus of stomach}
- {ko: 날문부분, en: pyloric part}
- {ko: 위몸통, en: body of stomach}
publishable: false
needs_review: false
extraction: drive-mcp-text
section: 위의 겉모습
---

## 구조물 용어 (4개)

- 들문부분 (cardial part)
- 위바닥 (fundus of stomach)
- 날문부분 (pyloric part)
- 위몸통 (body of stomach)

> 분류: region=abdomen (conf 1.0) · layers=['cavity-visceral'] · classes=-
> 원문 전문은 커밋하지 않는다(저작권) — `.private/anatomy/` 참조.

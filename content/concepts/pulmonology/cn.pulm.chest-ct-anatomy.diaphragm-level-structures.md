...
diagram:
  ...
  nodes:
    - {id: start, kind: start, text: "흉부 CT 축상면에서 둥근 연부조직 음영 — 창 설정과 단면 높이를 먼저 확인"}
    - {id: where, kind: decision, text: "어디에 있나? 척추체 바로 앞(세로칸) / 폐 바닥(가로막 근처)"}
    - {id: info, kind: info, text: "한 장만으로는 모른다 — 위아래 단면을 넘겨 연속성을 보고, 필요하면 세로칸창으로 바꿔 본다"}
    - {id: side, kind: decision, text: "척추체 왼쪽 앞의 크고 둥근 단면인가, 오른쪽 앞·정중의 납작한 관인가?"}
    - {id: aorta, kind: end, text: "하행대동맥 — 대동맥구멍 T12(가슴림프관·홀정맥 동반)"}
    - {id: eso, kind: end, text: "식도 — 식도구멍 T10(미주신경줄기 동반)"}
    - {id: dome, kind: decision, text: "매끈한 볼록 경계·균질·주변 폐 정상·아래 단면에서 간과 이어지나?"}
    - {id: normal, kind: end, text: "가로막돔과 간 — 정상(부분용적·기하학 효과)"}
    - {id: lesion, kind: alert, text: "병변 — 공기기관지조영상(경화)·부피 감소(무기폐)·뒤쪽 초승달(흉수)·불규칙 경계(종괴)로 가른다"}
  edges:
    - {from: start, to: where}
    - {from: where, to: side, label: "척추체 앞"}
    - {from: where, to: dome, label: "폐 바닥"}
    - {from: where, to: info, label: "한 단면뿐"}
    - {from: info, to: side, label: "세로칸 구조로 확인"}
    - {from: info, to: dome, label: "가로막 근처로 확인"}
    - {from: side, to: aorta, label: "왼쪽 앞·둥글고 큼"}
    - {from: side, to: eso, label: "오른쪽 앞·정중·납작"}
...
## 정의
이 정리본은 질환이 아니라 **정상 흉부 CT 해부**를 다룬다. 목표는 심실 높이·폐 바닥의 축상면에서 가로막 주변 정상 구조 — 하행대동맥·식도·홀정맥·가로막돔과 그 아래 간 — 를 위치·모양·위아래 연속성으로 식별해 병변과 가르고, 대동맥·식도·아래대정맥이 가로막을 지나는 높이(T12·T10·T8)에 연결하는 것이다. 영상 판독에서 「정상 구조를 병변으로 부르는 것」과 「구조를 잘못 식별해 엉뚱한 높이를 붙이는 것」이 가장 흔한 두 오류다.

## 병태생리
(정상 해부와 영상 원리) **가로막**은 가운데가 중심널힘줄, 가장자리가 근육인 돔 모양 판이다. 뒤쪽에서 두 다리(crura)가 허리뼈에 붙고, 두 다리를 잇는 정중활꼴인대가 대동맥 위로 아치를 이룬다. 오른쪽 돔은 아래에 간이 있어 왼쪽보다 높다.

세 구멍의 높이와 구조는 기능과 연결된다 [[?moore-coa]]. **대정맥구멍(T8)** 은 중심널힘줄 안에 있어 들숨에 가로막이 수축하면 오히려 벌어져 정맥 환류를 돕는다. **식도구멍(T10)** 은 오른다리 근육섬유가 감싸 수축할 때 식도를 조여 위 내용물 역류를 막는 생리적 조임근이 된다. **대동맥구멍(T12)** 은 가로막을 뚫지 않고 두 다리와 정중활꼴인대 **뒤**, 척추체 **앞**을 지나므로 가로막이 수축해도 대동맥이 눌리지 않는다 — 가슴림프관과 홀정맥이 함께 지난다.

**CT 원리** — CT 는 X선이 조직을 지나며 약해지는 정도를 하운스필드 단위로 재며 물은 0, 공기는 −1000 HU 다. 사람 눈은 비슷한 밀도를 잘 가르지 못하므로 창 너비·중심을 정해 보여 줄 범위를 고른다. 폐창에서는 폐보다 짙은 모든 구조가 희게, 세로칸창에서는 폐가 검게 보이며, 창을 바꿔도 복셀의 HU 값은 그대로다 [[harrison-21: 286장 p.2143]]. 흉부 사진이 3차원을 2차원으로 겹쳐 보여 주는 것과 달리 CT 는 흉곽의 모든 구조를 공간적으로 재구성한다 [[harrison-21: 286장 p.2143]] — 그러나 한 장의 축상면은 휘어진 장기를 얇게 자른 판이라, 휘어진 면(가로막돔)을 비스듬히 자르면 장기의 일부가 떨어진 「덩이」처럼 보인다(부분용적·기하학 효과).

## 기전에서 소견으로
- **폐창에서 세로칸이 희게 뭉침** → 창 설정의 결과이므로, 대동맥·식도·홀정맥은 밀도가 아니라 **자리와 모양**으로 가른다 [[harrison-21: 286장 p.2143]].
- **척추체 왼쪽 앞의 크고 둥근 단면** → 하행대동맥은 뒤세로칸에서 척추체 왼쪽 앞을 따라 내려가므로 위아래 모든 단면에서 같은 자리에 둥글게 잘린다.
- **대동맥의 오른쪽 앞·정중의 납작한 관** → 식도는 비어 있을 때 납작하고 안에 공기가 있을 수 있다.
- **척추체 오른쪽 앞의 작은 점** → 홀정맥.
- **폐 바닥 오른쪽 앞의 둥글고 균질한 음영** → 높은 오른쪽 돔의 꼭대기가 잘려 간이 폐로 둘러싸여 보인다. 경계는 휘어진 면이라 매끈하게 볼록하고, 속은 간 실질이라 완전히 균질하며, 아래 단면으로 갈수록 커져 간 전체와 이어진다 [[?webb-thoracic]].

## 감별
폐 바닥의 연부조직 음영은 가로막돔(정상)·폐엽 경화·무기폐·흉수·종괴를 가른다(표 참조). 핵심은 네 가지 질문이다 — 속에 공기기관지조영상이 있는가(경화), 부피가 줄어 틈새·폐문·세로칸이 끌려가는가(무기폐), 누운 자세의 뒤쪽 흉벽을 따라 초승달로 고였는가(흉수), 경계가 불규칙하고 아래 단면에서 간과 떨어진 독립 구조인가(종괴). 넷 모두 아니고 아래로 간과 이어지면 가로막돔이다. 증상이 없다는 병력은 경화의 가능성을 낮추지만, 흡연력 같은 병력이 영상 소견을 바꾸지는 않는다.

척추체 앞 구조의 감별은 대동맥(왼쪽 앞·둥글고 큼) · 식도(오른쪽 앞·정중·납작) · 홀정맥(오른쪽 앞·작음)이며, 심실 높이의 흉강에는 아래대정맥이 거의 보이지 않는다.

## 검사
1. **위아래 단면 넘겨 보기** — 한 장의 둥근 음영은 연속성을 봐야 정체가 드러난다. 가로막돔은 아래로 커져 간과 이어지고, 대동맥은 같은 자리에 이어진다.
2. **창 바꾸기** — 폐창은 폐 실질, 세로칸창은 세로칸 연부조직을 본다. 창은 HU 값을 바꾸지 않는다 [[harrison-21: 286장 p.2143]].
3. **관상·시상 재구성** — 가로막돔과 간의 연속성, 대동맥이 가로막 뒤로 내려가는 길을 한 장에 보여 준다.

## 치료
정상 구조이므로 치료 대상이 아니다. 이 목표에서 「처치」에 해당하는 판단은 **추가 검사를 하지 않는 것**이다 — 가로막돔을 종괴로 부르면 불필요한 추적 CT·조직검사로 이어지고, 대동맥을 식도로 부르면 엉뚱한 높이·구조를 보고한다. **반응 확인·재평가**: 판독을 마치기 전에 위아래 단면·다른 창·재구성 영상으로 연속성을 한 번 더 확인하고, 그래도 독립 구조로 남는 음영만 병변으로 기술한다.

## 권고와 예외
- 구조를 먼저 확정하고(자리·모양·연속성), 그다음에 높이를 붙인다 — 대동맥 T12, 식도 T10, 아래대정맥 T8.
- 폐 바닥 오른쪽 앞의 매끈하고 균질한 둥근 음영은 아래 단면에서 간과 이어지면 가로막돔이다. 한 장만으로 병변을 부르지 않는다.
- 해리슨 286장은 CT 의 감쇠·창 설정 원리까지만 다루고, 가로막 구멍 높이와 가로막돔 부분용적 효과는 다루지 않는다. 그 서술은 해부학·흉부영상 교과서(Moore·Webb)를 따랐으나 원문 쪽수는 대조하지 못했다(검토 항목).

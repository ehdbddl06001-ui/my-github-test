...
next_goal: "실험1′ 본 실행 — QUICK 파이프라인 검증 완료(114환자). QUICK=False(900환자)로 돌려 이소성 보유 테스트 환자를 14명 → 70명+ 로 올려야 B3R−B3 판정 가능. 노트북 notebooks/exp1_icentia_rhythm_scale.ipynb"
...
## 실험 1′ 진행 상황 (2026-07-31)

**파이프라인 검증 완료** — 예비 실행(Icentia11k 114환자·74,399비트·QUICK). 아래는 **판정용
수치가 아니다**(이소성 보유 테스트 환자 14명 → 사전등록 기준 20명 미달로 판정 거부됨).

- **물리 사전점검은 통과**: 조기박동에서 보상지수의 S↔V 분리 **Cohen's d = +0.772**.
  V는 IQR [−0.011, +0.018](완전 보상 → 0에 좁게 붙음), S는 IQR [−0.941, +0.001]
  (불완전 보상 → 음수로 넓게 퍼짐). **IQR 폭이 32배 차이** — 생리학 그대로다.
  선행 트랙(MIT-BIH/SVDB)과 **다른 기기·다른 인구·단일유도**에서 재현된 것이라 그 자체로 의미.
- **arm 순서도 선행과 같은 방향**: B2(형태만) < B3(+RR) < B3R(+RHYTHM).
  특히 B2에서 **S F1 0.004 vs V F1 0.258** — "S는 형태로 구별되지 않는다"는
  `PAPER.md` §8.1의 주장이 그대로 재현됐다.
- 예비 관찰(탐색적): 보상지수의 이득이 **S가 아니라 V 쪽에서** 나오는 것으로 보인다.
  보상지수가 V에서 훨씬 좁게 분포하므로 기전상 납득되지만, **확인은 본 실행에서** 한다.

**본 실행 조건**: `QUICK=False` → 900환자. 부트스트랩이 환자를 리샘플하므로 검정력은
분(minutes)이 아니라 **환자 수**에 달렸다. 900환자 → 테스트 315명 → 이소성 보유 약 110명.

**금지**: 위 예비 수치를 보고 하이퍼파라미터를 조정하지 말 것. 지금까지의 수정은 전부
**붕괴·NaN·정규화 오류 같은 고장**을 고친 것이지 효과 크기를 보고 바꾼 것이 아니다.
...
| **1′** | **Icentia11k 대규모 확증** ★ | **Icentia11k 400~900환자(단일유도, 공개)** | B3R−B3 = 보상지수 축의 순수 기여 | **95% CI가 0을 벗어나는가** (SVDB 73환자에선 CI 반폭 0.073 → 판정 불가였음) |

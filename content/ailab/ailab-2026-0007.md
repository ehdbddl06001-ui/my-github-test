...
related: [ailab-2026-0005, ailab-2026-0008, ailab-2026-0004]
---
...
결과(실행 로그): **`split=intra` · macro-F1 0.8273 · 통과**. (셀 7의 클래스별 F1 표는 이
심화 시점에 수치를 확보하지 못했다 — 지어내지 않는다. `inter`로 다시 돌려 로그로 남기면
클래스별로 정확히 붙일 수 있다.)
...
1. **[핵심] `intra` 분할은 같은 환자가 train/test에 섞인다(누수).** 기본값 `intra`는 44 레코드의
   ...
   태도다 — **남은 건 실제로 `inter`를 돌려 그 숫자를 로그로 박는 것**(E-1).
...
1. **`SPLIT="inter"`로 재측정** *(B-1,C-1)* — DS1/DS2로 한 줄 바꿔 돌린다.
   - 합격: `inter` 로그를 남기고 **intra 대비 macro-F1·S/F 하락폭**을 기록(떨어지는 게 정상).

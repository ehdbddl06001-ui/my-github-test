...
- 완료 후 `python pipelines/publish.py -m "KMLE <날짜>: …"` 로 검증·번들·main 푸시를 한 번에 한다.
--------------------------------------------------------------------

## 요일 분산 (권장 스케줄)
Claude Routine을 요일별로 나눠 각 프롬프트를 붙인다.
- **월·수·금**: 위 KMLE 프롬프트(이 파일).
- **화·목**: `prompts/routine_usmle.md` (USMLE 6문항, Step 1·2 분산, main 으로 게시).
- **일**: "오늘의 논문 3편을 gen-paper 규칙으로 요약", 커밋은 publish.py 로 main 에.
